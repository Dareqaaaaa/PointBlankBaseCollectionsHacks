...
{
	WinHandle = FindWindowW(NULL, Name.c_str());
	if (WinHandle)
	{
		RECT WinRect;
	...
	{
		MessageBoxA(0, "Make sure Point Blank is running!", "Error - Cannot find the game!", MB_OK | MB_ICONERROR);
		exit(1);
		return false;
	}
}
...
bool OverlayT::SetupOverlay()
{
	...
	Class.lpszClassName = Globals->OverlaySettings.Name;
