...
//char *argv[];
{
	...
	strcpy(reinterpret_cast<char*>(Dummy), argv[2]);
	strcat(reinterpret_cast<char*>(Dummy),".h");               /* add suffix .h to target name */

	if( (dest=fopen( reinterpret_cast<char*>(Dummy), "wb+" )) == NULL )
	{
	...
	strcpy(reinterpret_cast<char*>(Dummy),argv[3]);
	Upper_chars(reinterpret_cast<char*>(Dummy));  /* lower to upper chars */
	strcat(reinterpret_cast<char*>(Dummy),"_LEN");  /* add the suffix _LEN to the struct name */
